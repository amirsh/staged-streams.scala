...
class B {
   ...
   @Benchmark 
   def flatten3_baseline () : Int = {
      var sum = 0
      var counter1 = 0
      var counter2 = 0
      var counter3 = 0
      while(counter1 < nestedArr3.length) {
         val item1 = nestedArr3(counter1)
         while(counter2 < item1.length) {
            val item2 = item1(counter2)
            while(counter3 < item2.length) {
               val item3 = item2(counter3)
               sum += item3
               counter3 += 1
            }
            counter3 = 0
